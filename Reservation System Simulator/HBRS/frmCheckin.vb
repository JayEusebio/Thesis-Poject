...
Public Class frmCheckin
    ...
    Private Sub frmCheckin_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Dim a As String = MsgBox("Do you want to exit this form?", vbQuestion + vbYesNo, "Exit")
        If a = vbNo Then
            e.Cancel = True
        Else
            clear_text()
        ...
        Else
            ...
            If a = vbYes Then
                ...
                Else
                    ...
                    reportDocument.SetParameterValue("Transaction ID", txtTransID.Text)
                    reportDocument.SetParameterValue("Guest Name", txtGuestName.Text)
                    reportDocument.SetParameterValue("Room Number", txtRoomNumber.Text)
                    reportDocument.SetParameterValue("Room Type", txtRoomType.Text)
                    reportDocument.SetParameterValue("Room Rate", txtRoomRate.Text)
                    reportDocument.SetParameterValue("Check In Date", txtCheckInDate.Text)
                    reportDocument.SetParameterValue("Check Out Date", dtCheckOutDate.Text)
                    reportDocument.SetParameterValue("No. of Days", txtDaysNumber.Text)
                    reportDocument.SetParameterValue("No. of Adults", txtAdults.Text)
                    reportDocument.SetParameterValue("No. of Children", txtChildren.Text)
                    reportDocument.SetParameterValue("Discount Type", cboDiscount.Text)
                    reportDocument.SetParameterValue("Sub Total", txtSubTotal.Text)
                    reportDocument.SetParameterValue("Advance Payment", txtAdvance.Text)
                    reportDocument.SetParameterValue("Total Balance", txtTotal.Text)
